...
class data_loader:
    ...
    def __init__(self):
        ...
        self.dir = None # to be declared within the child class
    ...
    def download(self, url, file): 
        try:
            ...
            #print(f"Dataset downloaded and saved as {file}.")
    ...
    def extract_with_winrar(self, file):
        try:
            logging.info(f"Extracting dataset using WinRAR...")
            subprocess.run([self.winrar_path, 'x', file, self.dir], capture_output=True, text=True)
            logging.info(f"Dataset extracted to '{self.dir}'.")
            return True
        except Exception as e:
            logging.error(f"Error using WinRAR: {e}")
            return False

    def extract_with_7zip(self, file):
        try:
            logging.info(f"Extracting dataset using 7-Zip...")
            subprocess.run([self.sevenzip_path, 'x', '-aoa', file, f'-o{self.dir}'], capture_output=True, text=True)
            logging.info(f"Dataset extracted to '{self.dir}'.")
            return True
        except Exception as e:
    ...
    def normalize_and_slice(self, train_data, val_data, test_data, patch_size=32):
        ...
        dfs = []
        logging.info('Normalizing images and slicing into patches...')
        ...
        for (data, name) in sets:
            ...
            for row in data.itertuples(index=False):
                ...
                mos = row[1]
                distortion = row[2]
                image_path = f'{self.dir}/distorted_images/{filename}'
...
class tid2013_loader(data_loader):
    def __init__(self, download=True):
        ...
        self.rar_file = os.path.join(self.dir, 'tid2013.rar')
    ...
    def prepare_data(self, filter=True):
        ...
        train_data, val_data, test_data = self.normalize_and_slice(train_data, val_data, test_data)

        return [train_data, val_data, test_data]
    
#tid2013_loader(download=False)